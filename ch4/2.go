...
func F2() {
	...
	func() {
		...
		println("注意，可变参数包括0个参数的情形。可变参数只能是函数的最后一个参数。")
		println()
	...
	func() {
		...
		println("panic()会终止程序，后面的代码不会被执行。")
		println()
		println("不过，借助defer机制，可以使用recover()函数捕捉异常：")
		println("defer func() {")
		println("	str := recover()")
		println("	fmt.Println(str)")
		println("}()")
		func() {
			defer func() {
				str := recover()
